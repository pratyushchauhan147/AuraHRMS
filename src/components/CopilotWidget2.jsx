...
export default function CopilotWidget() {
  ...
  const [session, setSession] = useState({}); // Holds conversation context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // We send the prompt and the current session state to the backend
      const res = await fetch("/api/copilot", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ prompt: input, session: session }),
      });
      ...
      // We get the AI response AND the new session state back
      const aiMessage = { author: "ai", content: data.response };
      setMessages((prev) => [...prev, aiMessage]);
      setSession(data.session); // Save the updated session for the next turn
